...
void lifft_dct2(lifft_float_t x_in[], size_t stride_in, lifft_float_t x_out[], size_t stride_out, lifft_complex_t scratch[], size_t n){
	...
	for(size_t i = 0; i < n/4; i++){
		...
		scratch[_lifft_rev_bits18(n/2 - 1 - i, bits)] = lifft_complex(x_in[(4*i + 3)*stride_in], x_in[(4*i + 1)*stride_in]);
	}
...
void lifft_dct4(lifft_float_t x_in[], size_t stride_in, lifft_float_t x_out[], size_t stride_out, lifft_complex_t scratch[], size_t n){
	...
	lifft_complex_t w = lifft_complex(1, 0), wm = lifft_cispi((lifft_float_t)-2.0/n);
	for(size_t i = 0; i < n/4; i++){
		scratch[_lifft_rev_bits18(          i, bits)] = lifft_cmul(lifft_complex(x_in[(4*i + 0)*stride_in], x_in[(4*i + 2)*stride_in]), w);
		w = lifft_cmul(w, wm);
		scratch[_lifft_rev_bits18(n/2 - 1 - i, bits)] = lifft_cmul(lifft_complex(x_in[(4*i + 3)*stride_in], x_in[(4*i + 1)*stride_in]), lifft_conj(w));
	}
	
	_lifft_process(scratch, n/2);
	
	lifft_complex_t w0 = lifft_cispi((lifft_float_t)-0.5 + (lifft_float_t)-1.0/n), wm0 = lifft_cispi((lifft_float_t)-2.0/n);
	lifft_complex_t w1 = lifft_cispi((lifft_float_t)-0.25/n), wm1 = lifft_cispi((lifft_float_t)-0.5/n);
	w1 = lifft_cmul(w1, lifft_complex(1/sqrt(2*n), 0));
	for(size_t i0 = 0; i0 < n/2; i0++){
		lifft_complex_t p = scratch[i0], q = lifft_conj(scratch[n/2 - 1 - i0]);
		lifft_complex_t s = lifft_cadd(p, q), t = lifft_cmul(lifft_csub(p, q), w0);
		w0 = lifft_cmul(w0, wm0);
